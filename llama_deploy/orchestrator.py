...
def _print_summary(cfg, api_token: str, first_run: bool, unverified_models: list) -> None:
    ...
    print()
    # public_base_url already ends in /v1 (e.g. "https://domain/v1").
    # Strip it so we can append paths without creating /v1/v1/...
    _pub = cfg.public_base_url.rstrip("/")
    origin   = _pub[:-3] if _pub.endswith("/v1") else _pub   # bare https://domain
    base_url = origin + "/v1"
    ...
    print()
    print("  # Embeddings")
    print(f"  curl {base_url}/embeddings \\")
    print(f"    -H 'Authorization: Bearer {tok}' \\")
    print( "    -H 'Content-Type: application/json' \\")
    ...
    print(f"  client = OpenAI(base_url=\"{base_url}\", api_key=\"{tok}\")")
